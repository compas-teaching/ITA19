...
def draw_frame(frame, layer):
    ...
    points = []
    lines = []
    points.append({'pos': origin, 'color': (0, 255, 255)})
    lines.append({'start': origin, 'end': xaxis, 'color': (255, 0, 0)})
    lines.append({'start': origin, 'end': yaxis, 'color': (0, 255, 0)})
    lines.append({'start': origin, 'end': zaxis, 'color': (0, 0, 255)})
    compas_rhino.draw_points(points, layer=layer, clear=True)
